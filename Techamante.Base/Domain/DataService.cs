...
{
    public abstract class DataService<TEntity> : IDataService<TEntity> where TEntity : BaseEntity
    {
        ...
        #region Constructor
        protected DataService(IRepositoryAsync<TEntity> repository) { _repository = repository; }
        #endregion Constructor

        public virtual TEntity Find(params object[] keyValues) { return _repository.Find(keyValues); }

        public virtual IQueryable<TEntity> SelectQuery(string query, params object[] parameters) { return _repository.Query(query, parameters).AsQueryable(); }
